...
public class KubernetesAppScheduler implements Scheduler {
	...
	public KubernetesAppScheduler(BatchV1beta1Api batchV1beta1Api) {
	...
	public void schedule(ScheduleRequest scheduleRequest) {
		...
		try {
			batchV1beta1Api.createNamespacedCronJob(namespace, body, pretty);
		}
	...
	public void unschedule(String scheduleName) {
		...
		body.setApiVersion("batch/v1beta1");
		body.setGracePeriodSeconds(5l);
		body.setOrphanDependents(false);
		try {
			V1Status result = batchV1beta1Api.deleteNamespacedCronJob(scheduleName, "default", body, null, 5, false, null);
	...
	private V1PodSpec createPodSpec(ScheduleRequest request) {
		...
		try {
			container.setImage(request.getResource().getURI().getSchemeSpecificPart());//"cppwfs/timestamp-task");
		}
		catch (IOException e) {
